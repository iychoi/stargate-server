...
import stargate.commons.utils.RandomUtils;
 ...
 * @author iychoi
 */
public class RandomContactNodeSelectionAlgorithm extends AbstractContactNodeSelectionAlgorithm {
    ...
    public RandomContactNodeSelectionAlgorithm(StargateService service, TransportManager manager) {
        if(service == null) {
            throw new IllegalArgumentException("service is null");
        }
    ...
    public synchronized ResponsibleNodeMapping getResponsibleRemoteNodeMappings(Cluster localCluster, Cluster remoteCluster) throws IOException {
        ...
        if(mappings == null) {
            ...
            for(int i=0;i<localNodeNum;i++) {
                ...
                int randomNodeIdx = RandomUtils.getRandomNumber(0, remoteNodeNum);
                Node remoteNode = remoteNodes.get(randomNodeIdx);
